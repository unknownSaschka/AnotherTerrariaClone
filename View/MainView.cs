...
namespace ITProject.View
{
    ...
    class MainView : GameWindow
    {
        ...
        private int _fontVBO;    //Vertex Buffer Object
        private int _fontVAO;   //Vertex Array Object
        ...
        {
            _fonts = new Fonts("fonts/Depredationpixie.ttf");

            GL.GenVertexArrays(1, out _fontVAO);
            GL.GenBuffers(1, out _fontVBO);
        ...
        private void DrawGUI()
        {
            ...
            RenderText(_fontShader, "Test", 0, 0, 5, new Vector4(1.0f, 1.0f, 1.0f, 0.0f));
        }
        ...
        private void DrawWorldV2()
        {
            ...
            _shader.Use();
            _shader.SetMatrix4("transform", Matrix4.CreateTranslation(new Vector3(smoothCameraPos.X, smoothCameraPos.Y, 0f)));
            _shader.SetVector4("color", new Vector4(1f, 1f, 1f, 1f));

            DrawPlayer(player.Position.X, player.Position.Y, player.Size.X, player.Size.Y);

            for (int iy = (int)(player.Position.Y - _renderDistance.Y); iy < player.Position.Y + _renderDistance.Y + 1; iy++)
            {
                for (int ix = (int)(player.Position.X - _renderDistance.X); ix < player.Position.X + _renderDistance.X + 1; ix++)
                {
        ...
        private void DrawTexturedRect(Box2D box, int textureID)
        {
        ...
        private void RenderText(Shader shader, string text, float x, float y, float scale, Vector4 color)
        {
            GL.Enable(EnableCap.Blend);
            GL.BlendFunc(BlendingFactor.SrcAlpha, BlendingFactor.OneMinusSrcAlpha);
            ...
            Matrix4 projection = Matrix4.CreateOrthographic(Width, Height, 0, 1);
            ...
            for(int i = 0; i < text.Length; i++)
            {
                ...
                //Cursor für den nächsten Character ausrichten
